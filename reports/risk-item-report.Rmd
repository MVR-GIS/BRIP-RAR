...
# Filter the events df for the current risk item
events_item <- events %>%
  filter(TABLE_NAME == "RISK_REGISTER") %>%
  filter(fk_table_id == x)

# Filter the discussion df for the current risk item
discussion_item <- discussion %>%
  filter(TABLE_NAME == "RISK_REGISTER") %>%
  filter(fk_table_id == x)

# Filter the related risks df for the current risk item
risk_rel_risk_item <- risk_rel_risk %>%
  filter(risk_no == x)
```
...
**Related Risks**
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
if(length(risk_rel_risk_item$risk_no) > 0) {
  kbl(select(risk_rel_risk_item, related_risk_link, RELATED_RISK_CONCERN),
...
<!-- Decisions table goes here -->

:::::: {.white-box}
`r cat(' ')`
::::::

<!-- End Related Decisions -->
:::::::::  
