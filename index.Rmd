...
```{r wrangle-related-risk, echo=FALSE, warning=FALSE, message=FALSE}
risk_related_oracle <- read_csv(file = "data/REL_RISK_RISK_MAIN_VIEW_20211126.csv")

# Filter for "Active" records
risk_related <- risk_related_oracle %>%
  filter(ACTIVE == "Yes")

# Convert factor variables to title case
risk_related$RELATED_RISK_CATEGORY <- toTitleCase(tolower(risk_related$RELATED_RISK_CATEGORY))
...
# Cleanup related risk number for sorting
risk_related <- risk_related %>%
  mutate(rel_risk_no_risk_cat_code = str_extract(RELATED_RISK, "\\w+\\b")) %>%
  mutate(rel_risk_no_number_string = str_extract(RELATED_RISK, "\\b\\w+$")) %>%
  mutate(rel_risk_no_number = str_extract(rel_risk_no_number_string, "\\d+")) %>%
  mutate(rel_risk_no_letter_na = str_extract(rel_risk_no_number_string, "\\D+")) %>%
  mutate(rel_risk_no_letter = replace_na(rel_risk_no_letter_na,
                                         replace = "")) %>%
  mutate(rel_risk_no_letter_lower = tolower(rel_risk_no_letter)) %>%
  mutate(rel_risk_no_padded = str_pad(rel_risk_no_number, 
                                      width = 3, 
                                      side = "left",
                                      pad = "0")) %>%
  mutate(related_risk = str_c(rel_risk_no_risk_cat_code, 
                          "-",
                          rel_risk_no_padded,
                          rel_risk_no_letter_lower))

# Remove unused fields
risk_related <- risk_related %>%
  select(!c(risk_no_risk_cat_code, 
            risk_no_number_string, risk_no_number,
            risk_no_letter_na, risk_no_letter_lower,
            risk_no_letter, risk_no_padded,
            rel_risk_no_risk_cat_code,
            rel_risk_no_number_string, rel_risk_no_number,
            rel_risk_no_letter_na, rel_risk_no_letter_lower,
            rel_risk_no_letter, rel_risk_no_padded))

# Create hyperlink to Related Risk
risk_related <- risk_related %>%
  mutate(related_risk_link = paste0("[", risk_related$related_risk, "](#risk-item-", 
                               tolower(risk_related$related_risk), ")"))

# Reorder fields
risk_related <- risk_related %>%
  relocate(risk_no, .after = RISK_NO) %>%
  relocate(related_risk, .after = RELATED_RISK) %>%
  relocate(related_risk_link, .after = related_risk)

# Cleanup
#rm(risk_related_oracle)
```
...
```{r wrangle-related-action, echo=FALSE, warning=FALSE, message=FALSE}
action_related_oracle <- read_csv(file = "data/REL_RISK_ACTION_MAIN_VIEW_20211126.csv")

# Filter for "Active" records
action_related <- action_related_oracle %>%
  filter(ACTIVE == "Yes")
```
