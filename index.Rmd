...
events <- events_oracle %>%
  ...
  mutate(last_edited_date = lubridate::as_datetime(LAST_EDITED_DATE)) 
...
# Remove unused fields
events <- events %>%
  select(!c(fk_table_id_cat_code, fk_table_id_number_string, fk_table_id_number,
...
discussion <- discussion_oracle %>%
  ...
  mutate(created_date   = lubridate::as_datetime(CREATED_DATE)) %>%
  mutate(last_edited_date = lubridate::as_datetime(LAST_EDITED_DATE)) %>%
  mutate(poc_review_date = lubridate::as_datetime(POC_REVIEW_DATE))
```
