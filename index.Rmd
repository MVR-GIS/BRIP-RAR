...
# Remove test data
risk <- risk %>%
  filter(!(str_extract(tolower(RISK_NO), "test") %in% "test"))
...
```{r create-edgebundleR, echo=FALSE, warning=FALSE, message=FALSE}
...
# Identify blocks
itemBlocks <- cohesive_blocks(simplify(relate_igraph))
blocks_df <- as.data.frame(t(sapply(itemBlocks$blocks, as_ids)))
#plot(itemBlocks, relate_igraph)
```
